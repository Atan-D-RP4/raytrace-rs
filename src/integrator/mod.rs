...
mod tests {
    ...
    fn qmc_dim_invariant() {
        ...
        let floor_mat = Material::lambertian_color(0.7, 0.7, 0.7);
        let mirror_mat = Material::metal(Vec3::new(1.0, 1.0, 1.0), 0.0); // fuzz=0 => delta
        let light_mat = Material::light(Vec3::new(8.0, 8.0, 8.0));
        ...
        let integrator = PathTracingIntegrator::new(1, Color3::ZERO, None); // max_depth=1
        ...
        // Expected: RR(1) + material(6) + delta pad(4) = 11
        {
            ...
            let mut ray = Ray::new_with_time(Vec3::new(0., 0., 4.), dir, 0.0);
            let _ = integrator.li(&mut ray, &world, &lights, &mut dim_cursor);
            assert_eq!(
                dim_cursor.offset(),
                11,
        ...
        // Expected: 0 dims consumed (early return before any sample calls)
        {
            let sampler = NaiveRandomSampler::with_seed(42);
            let mut dim_cursor = DimCursor::new(0, sampler);
            let dir = Vec3::new(0.0, 1.0, 0.0); // straight up — misses everything
            let mut ray = Ray::new_with_time(Vec3::new(0., 0., 4.), dir, 0.0);
            let _ = integrator.li(&mut ray, &world, &lights, &mut dim_cursor);
